...
class WeworkCorp {
  ...
  getAuthorizeUrl(redirectUri, state, scope = 'snsapi_userinfo') {
    return `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${this.config.corpid}&redirect_uri=${redirectUri}&response_type=code&scope=${scope}&agentid=${this.config.agentid}&state=${state}#wechat_redirect`;
  }
}
